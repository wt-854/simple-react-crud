...
const EditForm = ({ currentContact }) => {
  ...
  const [phoneNumbers, setPhoneNumbers] = useState(currentContact.phoneNumbers);
  ...
  const myHandleSubmit = (data) => {
    ...
    contacts.forEach((contact) => {
      if (contact.email === data.email && data.email !== currentEmail) {
        uniqueEmail = false;
      }
    });
  ...
  return (
    <Form onSubmit={handleSubmit(myHandleSubmit)}>
      ...
      <Alert show={uniqueEmailAlert} variant='danger'>
        Unable to update! A record with this email already exists!
      </Alert>
